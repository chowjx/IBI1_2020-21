...
covid_data.head(5) # Get first 5 rows of covid_data dataframe
covid_data.info() # Get info of data type of data points in covid_data (output printed)
print(covid_data.describe()) # Get summary statistics of covid_data dataframe (output printed)

covid_data.iloc[0,1] # Get data from 1st row, 2nd column by row & column index
covid_data.iloc[2,0:5] # Get data from 3rd row, from columns 0 to 5
covid_data.iloc[0:2,:] # Get data from first 2 rows & all columns
covid_data.iloc[0:10:2,0:5] # Get data from every second row between & including 0 & 8, & from columns 0 to 5
...
covid_data.loc[2:4,"date"] # Get data from row 2 to 4 & column named "date"
print(covid_data.loc[0:81,["total_cases"]]) # Get data from row 0 to 81 & column named "total_cases" (output printed)

# Get total cases from Afghanistan only (same output asc ovid_data.loc[0:81,"total_cases"]) (output printed)
...
print('world mean', np.mean(new_cases_world)) # Get mean of new cases from World (output printed)
print('world median new_cases', np.median(new_cases_world)) # Get median of new cases from World (output printed)
...
# Question : Plot boxplot of total cases in different countries (excluding World) on 14 March 2020
diff_total_cases = covid_data.loc[(covid_data['date'] == '2020-03-14') & (covid_data['location'] != 'World'), ['date','location','total_cases']]
print(diff_total_cases) # (output printed)
